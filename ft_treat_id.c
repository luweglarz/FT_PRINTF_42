...
/*   Updated: 2020/07/30 14:45:22 by lweglarz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_print_integer(int nbr, t_struct *strct)
{
	...
	if ((neg == 1 && strct->precisneg == 1 && strct->zero == 1) ||
	   ( neg == 1 && strct->zero == 1 && strct->dot == 0))
	{
		hasneg = 1;
		ft_putchar('-', strct);
	}
	if (strct->minus == 1)
		ft_put_minus_integer(str, strct, neg, hasneg);
	if (strct->dot == 1 && strct->precisneg == 0)
